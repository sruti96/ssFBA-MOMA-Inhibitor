include("Kinetics.jl")
function Bounds_TTA(data_dictionary,TXTL_dictionary,species_array,idx)

  FB = data_dictionary["flux_bounds_array"]
  ...
  ConcDict = TXTL_dictionary["ConcDictionary"]
  ...
  flux_mrna = ConcDict["Flux_mrna"]

  #==================================Flux RATES===========================================#
  KineticFlux = calculate_kinetic_limits(TXTL_dictionary,species)
  FB[1:200,2] .= data_dictionary["Enzyme_sample"] .* KineticFlux[1:200]
  ...
  for i = 1:length(rxn_enzyme_idx)
    if idx <= 60
      FB[rxn_enzyme_idx[i],2] = data_dictionary["Enzyme_sample"] * vmax_2h[i]
    elseif idx > 60
      FB[rxn_enzyme_idx[i],2] = data_dictionary["Enzyme_sample"] * vmax_8h[i]
    end
  end

  #===============================Update Species Bound Array========================================#
  #Match FB (flux bounds) constraints to SBA (species bounds)
  network = data_dictionary["stoichiometric_matrix"];
  ...
  aa_tmp = findall(-0.01.<aa_flux[idx,:].<0.06) # TTA
  SBA[aa_idx[aa_id_pos],1] .= (0.0) .* aa_flux[idx,aa_id_pos]
  SBA[aa_idx[aa_id_pos],2] .= data_dictionary["AA_pos"][aa_id_pos] .* aa_flux[idx,aa_id_pos]
  SBA[aa_idx[aa_id_neg],1] .= max.(-species[aa_idx[aa_id_neg]],data_dictionary["AA_neg"][aa_id_neg] .* aa_flux[idx,aa_id_neg])
  SBA[aa_idx[aa_id_neg],2] .= max.(-species[aa_idx[aa_id_neg]],(data_dictionary["AA_neg"][aa_id_neg] .- delta).*aa_flux[idx,aa_id_neg])
  #Allow for expiremental noise
  SBA[aa_idx[aa_tmp],1] .= max.(-species[aa_idx[aa_tmp]],data_dictionary["AA_pos"][aa_tmp] .* (-0.015))
  SBA[aa_idx[aa_tmp],2] .= 0.0.*data_dictionary["AA_pos"][aa_tmp] .* aa_flux[idx,aa_tmp]
  ...
  SBA[tca_idx[tca_id_pos],2] .= data_dictionary["TCA_pos"][tca_id_pos] .* tca_flux[idx,tca_id_pos]
  SBA[tca_idx[tca_id_neg],1] .= max.(-species[tca_idx[tca_id_neg]],data_dictionary["TCA_neg"][tca_id_neg] .* tca_flux[idx,tca_id_neg])
  SBA[tca_idx[tca_id_neg],2] .= max.(-species[tca_idx[tca_id_neg]],(data_dictionary["TCA_neg"][tca_id_neg] .- delta).*tca_flux[idx,tca_id_neg])
  ...
  SBA[gly_idx[gly_id_pos],2] .= data_dictionary["GLY_pos"][gly_id_pos] .* gly_flux[idx,gly_id_pos]
  SBA[gly_idx[gly_id_neg],1] .= max.(-species[gly_idx[gly_id_neg]],data_dictionary["GLY_neg"][gly_id_neg] .* gly_flux[idx,gly_id_neg])
  SBA[gly_idx[gly_id_neg],2] .= max.(-species[gly_idx[gly_id_neg]],(data_dictionary["GLY_neg"][gly_id_neg] .- delta).*gly_flux[idx,gly_id_neg])
  ...
  SBA[reducing_idx[reducing_id_pos],2] .= data_dictionary["REDUCING_pos"][reducing_id_pos] .* reducing_flux[idx,reducing_id_pos]
  SBA[reducing_idx[reducing_id_neg],1] .= max.(-species[reducing_idx[reducing_id_neg]], data_dictionary["REDUCING_neg"][reducing_id_neg] .* reducing_flux[idx,reducing_id_neg])
  ...
  data_dictionary["flux_bounds_array"] = FB
