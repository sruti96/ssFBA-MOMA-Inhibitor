include("Kinetics.jl")
function Bounds_DNP(data_dictionary,TXTL_dictionary,species_array,idx)
  ...
  aa_tmp = findall(-0.02.<aa_flux[idx,:].<0.0) # DNP
  ...
  SBA[aa_idx[aa_tmp],1] .= max.(-species[aa_idx[aa_tmp]],data_dictionary["AA_pos"][aa_tmp] .* (-0.02))
  ...
  energy_tmp = findall(-0.025.<energy_flux[idx,:].<0)
