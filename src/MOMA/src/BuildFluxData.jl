...
function BuildFluxData(TXTL,case,t_array)
    ...
    data_mrna = DataFrame(CSV.File("FinalDATA/Data/$(case)_mRNA_protein.dat"))[1:5,1]
    #deletecols!(data_met,:FAD)

    no_mets = length(data_met[1,:])
    no_aa = length(data_aa[1,:])
    no_mrna = length(data_mrna[1,:])
    ...
    sim_mrna = zeros(length(t_vec),no_mrna)
    for i = 1:no_mrna
        sim_mrna[:,i] = np.interp(t_vec,data_t,data_mrna[:,i])
    end
    ...
    smooth_mrna = zeros(length(t_vec),no_mrna)
    for i = 1:no_mrna
        spl_mrna = fit(SmoothingSpline,t_vec,sim_mrna[:,i],100.0)
        y_mrna = SmoothingSplines.predict(spl_mrna)
        diff = y_mrna[1] - sim_mrna[1,i]
        smooth_mrna[:,i] = y_mrna .- diff
    end
    # print(length(smooth_mrna))
    ...
    sim_mrna_smooth = zeros(length(t_array),no_mrna)
    for i = 1:no_mrna
        sim_mrna_smooth[:,i] = np.interp(t_array,t_vec,smooth_mrna[:,i])
    end
    ...
    flux_mrna = zeros(no_data_points-1,no_mrna)
    for flux_idx = 1:(no_data_points-1)
        ...
        flux_mrna[flux_idx,:] .= (sim_mrna_smooth[flux_idx+1,:] - sim_mrna_smooth[flux_idx,:])/(t_array[flux_idx+1]-t_array[flux_idx])
    end
    #================Find Species Index=======================#
    ...
    #==================Store In Dictionary========================#
    ConcData_dictionary  = Dict{String,Any}()
    ...
    ConcData_dictionary["Flux_mrna"] = flux_mrna
