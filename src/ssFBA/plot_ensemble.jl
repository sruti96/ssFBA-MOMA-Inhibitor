...
for idx = 1:2
    ...
    if idx == 1
        if plotline == 1
             plot(t_sim,Species_mean[:,gfp_sim_idx[idx]]*1e6,color=lcolor,linestyle=linestyle)
        end
        fill_between(t_sim,Species_pos[:,gfp_sim_idx[idx]]*1e6,Species_neg[:,gfp_sim_idx[idx]]*1e6,color=shade,alpha=0.5,linewidth=0)

    else
        if plotline == 1
            plot(t_sim,Species_mean[:,gfp_sim_idx[idx]]*1e3,color=lcolor,linestyle=linestyle)
        end
...
for idx = 1:20
    ...
    fill_between(t_sim,Species_pos[:,aa_sim_idx[idx]],Species_neg[:,aa_sim_idx[idx]],color=shade,linewidth=0,alpha = 0.5)
    if plotline == 1
...
tight_layout()
if plotsave == 1
    savefig("$(filedir)/$(savecase)_aa.pdf")
    close()
end
#--------------------Energy Species---------------------------#
row = 3
...
for idx = 1:12
    ...
    fill_between(t_sim,Species_pos[:,energy_sim_idx[idx][1]],Species_neg[:,energy_sim_idx[idx][1]],color=shade,linewidth=0,alpha = 0.5)
    if plotline == 1
...
tight_layout()
if plotsave == 1
    savefig("$(filedir)/$(savecase)_energy.pdf")
    close()
end
#--------------------Glycolysis---------------------------#
row = 4
...
for idx = 1:12
    ...
    fill_between(t_sim,Species_pos[:,gly_sim_idx[idx][1]],Species_neg[:,gly_sim_idx[idx][1]],color=shade,linewidth=0,alpha = 0.5)
    if plotline == 1
...
tight_layout()
if plotsave == 1
    savefig("$(filedir)/$(savecase)_gly.pdf")
    close()
end
#--------------------Reducing Power---------------------------#
row = 2
...
reducing = DataFrame(CSV.File("FinalDATA/Data/$(case)_reducing.dat"))
...
for idx = 1:4
    ...
    fill_between(t_sim,Species_pos[:,reducing_sim_idx[idx][1]],Species_neg[:,reducing_sim_idx[idx][1]],color=shade,linewidth=0,alpha = 0.5)
    if plotline == 1
...
tight_layout()
if plotsave == 1
    savefig("$(filedir)/$(savecase)_nad.pdf")
    close()
end
#-------------------TCA---------------------------#
row = 4
...
for idx = 1:12
    ...
    fill_between(t_sim,Species_pos[:,tca_sim_idx[idx][1]],Species_neg[:,tca_sim_idx[idx][1]],color=shade,linewidth=0,alpha = 0.5)
    if plotline == 1
...
tight_layout()
if plotsave == 1
    savefig("$(filedir)/$(savecase)_tca.pdf")
    close()
end
